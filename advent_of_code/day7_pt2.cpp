...
Hands parse_hand(std::string hand) {
  ...
  if (jokers == 5 || sorted_freq[0].first == 5 || sorted_freq[0].first + jokers == 5) {
    return FIVE_OF_A_KIND;
  } else if (sorted_freq[0].first == 4 || sorted_freq[0].first + jokers == 4) {
    return FOUR_OF_A_KIND;
  } else if ((sorted_freq[0].first == 3 && sorted_freq[1].first == 2) || (sorted_freq[0].first == 2 && sorted_freq[1].first == 2 && jokers == 1)) {
    return FULL_HOUSE;
  } else if (sorted_freq[0].first + jokers == 3) {
...
int main() {
  ...
  for (size_t i = map.size(); i > 0; i--) {
    std::cout << map[map.size() - i].first << " " << hand_names[parse_hand(map[map.size() - i].first)] << ": " << i << "*" << map[map.size() - i].second << std::endl;
